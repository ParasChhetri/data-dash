...
export const Testimonials = () => {
    ...
    const randomRotations = useMemo(() => generateRandomRotations(images.length), []);

    return (
        <div className="min-h-screen w-full flex justify-center items-center flex-col bg-blue-50/25">
            <h1 className="text-2xl font-medium text-[#0D1B2A] md:text-4xl md:font-semibold lg:text-6xl lg:font-extrabold mb-8">What they say about us</h1>
            {/* Container */}
            ...
            ${index === indexNum ? "opacity-100 scale-100 rotate-0 z-10" : "opacity-0 scale-90"}`}
                                style={{
                                    transform: `rotate(${index === indexNum ? 0 : randomRotations[index]}deg)`,
                                }}
                            />
                        ))}
                    </div>
                    ...
                    <div className="relative sm:w-[400px] w-[320px] mt-22 lg:mt-5 text-center">
                        {descriptions.map((description, index) => (
                            <p key={index} className={`absolute transition-opacity duration-500 ${index === indexNum ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                                {description}
                            </p>
                        ))}
                    </div>
                ...
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 flex gap-x-4 mt-5">
                    <button
                        ...
                        onClick={() => setIndexNum((prev) => (prev === 0 ? images.length - 1 : prev - 1))}
                    >
                        <MdArrowBackIos size={24} />
                    </button>
                    <button
                        className="cursor-pointer p-2 rounded-full hover:bg-gray-300"
                        onClick={() => setIndexNum((prev) => (prev === images.length - 1 ? 0 : prev + 1))}
                    >
